using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

#pragma warning disable CA1814 // Prefer jagged arrays over multidimensional

namespace BusinessObjects.Migrations
{
    /// <inheritdoc />
    public partial class SeedData_01_09272024 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropColumn(
                name: "DailyTicketId",
                table: "DailyTour");

            migrationBuilder.InsertData(
                table: "Account",
                columns: new[] { "Id", "AccessFailedCount", "Address", "AvatarUrl", "ConcurrencyStamp", "CreateDate", "Dob", "Email", "EmailConfirmed", "FullName", "Gender", "LockoutEnabled", "LockoutEnd", "NormalizedEmail", "NormalizedUserName", "PasswordHash", "PhoneNumber", "PhoneNumberConfirmed", "RefreshToken", "RefreshTokenExpiryTime", "Roles", "SecurityStamp", "Status", "TwoFactorEnabled", "UpdateDate", "UserName", "ZaloUser" },
                values: new object[] { "7e8f8c33-6506-4c2d-9258-c01f8c69413b", 0, "Quận 10, Hồ chí minh", "data:image/png;base64,iVBO", "862e310c-1810-45d5-b0f2-1645e40d0e3f", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7103), new DateTime(1990, 5, 15, 0, 0, 0, 0, DateTimeKind.Unspecified), "huahao04@gmail.com", false, "Hứa Thượng Hảo", true, false, null, null, null, null, "0395840777", false, null, null, null, "18f165fc-3ef5-4a38-b780-bfe4729710ad", 0, false, null, null, "" });

            migrationBuilder.InsertData(
                table: "City",
                columns: new[] { "CityId", "CityName", "CreateDate", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "Hồ Chí Minh", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(6693), 0, null },
                    { "2", "Hà Nội", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(6710), 1, null }
                });

            migrationBuilder.InsertData(
                table: "POIType",
                columns: new[] { "POITypeId", "CreateDate", "POITypeName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7447), "Chợ", 0, null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7450), "Bảo tàng", 1, null }
                });

            migrationBuilder.InsertData(
                table: "PaymentMethod",
                columns: new[] { "PaymentId", "CreateDate", "PaymentType", "Status", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7792), "Momo", 0, null });

            migrationBuilder.InsertData(
                table: "RequestType",
                columns: new[] { "RequestTypeId", "CreateDate", "Priority", "Status", "Type", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7967), 1, 0, "Support", null });

            migrationBuilder.InsertData(
                table: "Revenue",
                columns: new[] { "RevenueId", "CreateDate", "RevenueDate", "Status", "TotalRevenue", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7762), new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7760), 0, 900000f, null });

            migrationBuilder.InsertData(
                table: "ServiceType",
                columns: new[] { "ServiceTypeId", "CreateDate", "ServiceTypeName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7204), "Tour", 0, null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7206), "Hotel", 1, null }
                });

            migrationBuilder.InsertData(
                table: "Supplier",
                columns: new[] { "SupplierId", "CreateDate", "Status", "SupplierName", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7172), 0, "FPT", null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7174), 1, "VNPT", null }
                });

            migrationBuilder.InsertData(
                table: "CustomerSupport",
                columns: new[] { "CusSupportId", "CreateDate", "DateResolved", "Description", "RequestTypeId", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7942), new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7942), "Support", "1", 0, null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "Destination",
                columns: new[] { "DestinationId", "CityId", "CreateDate", "DestinationName", "PriceDestination", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7345), "Quận 1", 390000f, 0, null },
                    { "2", "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7348), "Quận 2", 350000f, 1, null }
                });

            migrationBuilder.InsertData(
                table: "Notification",
                columns: new[] { "NotifyId", "CreateDate", "Message", "SendDate", "Status", "Title", "Type", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7912), "Chúc mừng bạn đã đặt tour thành công", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7911), 0, "Thành công", "Success", null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "PackageTour",
                columns: new[] { "PackageTourId", "CityId", "CreateDate", "PackageTourName", "PackageTourPrice", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7488), "Tour Hồ Chí Minh", 500000f, 0, null },
                    { "2", "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7491), "Tour Hà Nội", 600000f, 1, null }
                });

            migrationBuilder.InsertData(
                table: "Service",
                columns: new[] { "ServiceId", "CreateDate", "LocationId", "ServiceName", "ServicePrice", "ServiceTypeId", "Status", "SupplierId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7245), null, "Tour Hồ Chí Minh", null, "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7249), null, "Tour Hà Nội", null, "1", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "DailyTour",
                columns: new[] { "DailyTourId", "CreateDate", "DailyTourName", "DailyTourPrice", "Description", "Discount", "EndDate", "ImgUrl", "PackageTourId", "StartDate", "Status", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7565), "Tour Hồ Chí Minh", 790000f, "Tour tham quan du lịch tp Hồ Chí Minh", 10, new DateTime(2024, 9, 12, 17, 30, 0, 0, DateTimeKind.Unspecified), "data:image/jpeg;base64,", "1", new DateTime(2024, 9, 12, 14, 30, 0, 0, DateTimeKind.Unspecified), 0, null });

            migrationBuilder.InsertData(
                table: "Location",
                columns: new[] { "LocationId", "CreateDate", "DestinationId", "LocationName", "LocationType", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7381), "1", "Nhà hàng", 1, 0, null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7383), "1", "Khách sạn", 1, 1, null }
                });

            migrationBuilder.InsertData(
                table: "TicketType",
                columns: new[] { "TicketTypeId", "CreateDate", "PackageTourId", "Status", "TicketTypeName", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7599), "1", 0, "Vé người lớn (>=16 tuổi)", null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7601), "1", 1, "Vé trẻ em", null }
                });

            migrationBuilder.InsertData(
                table: "TourSegment",
                columns: new[] { "TourSegmentId", "CreateDate", "DestinationId", "PackageTourId", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7311), "1", "1", 0, null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7313), "2", "2", 1, null }
                });

            migrationBuilder.InsertData(
                table: "Booking",
                columns: new[] { "BookingId", "BookingDate", "CreateDate", "DailyTourId", "PaymentId", "Status", "TotalPrice", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7632), new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7634), "1", "1", 0, 500000f, null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "DailyTicket",
                columns: new[] { "DailyTicketId", "Capacity", "CreateDate", "DailyTourId", "Status", "TicketTypeId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", 10, new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7526), "1", 0, "1", null },
                    { "2", 10, new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7529), "1", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "PointOfInterest",
                columns: new[] { "PointId", "CreateDate", "LocationId", "POITypeId", "PointName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7410), "1", "1", "Chợ Bến Thành", 0, null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7418), "1", "2", "Bảo tàng Hồ Chí Minh", 1, null }
                });

            migrationBuilder.InsertData(
                table: "ServiceByTourSegment",
                columns: new[] { "SBTSId", "CreateDate", "ServiceId", "Status", "TourSegmentId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7276), "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7279), "2", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "BookingByRevenue",
                columns: new[] { "BookingByRevenueId", "BookingId", "CreateDate", "RevenueId", "Status", "UpdateDate" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7729), "1", 0, null });

            migrationBuilder.InsertData(
                table: "Feedback",
                columns: new[] { "FeedbackId", "BookingId", "CreateDate", "FeedbackMsg", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7851), "Rất tuyệt vời", 0, null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "Rate",
                columns: new[] { "RateId", "BookingId", "CreateDate", "RateStar", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7879), 5, 0, null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "Ticket",
                columns: new[] { "TicketId", "BookingId", "CreateDate", "Price", "QR", "Quantity", "Status", "TicketName", "TicketTypeId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7667), 100000f, "", 10, 0, "Vé vào cổng", "1", null },
                    { "2", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7670), 50000f, "", 10, 1, "Vé ăn uống", "1", null }
                });

            migrationBuilder.InsertData(
                table: "TransactionsHistory",
                columns: new[] { "TransactionId", "BookingId", "CreateDate", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7822), 0, null, "7e8f8c33-6506-4c2d-9258-c01f8c69413b" });

            migrationBuilder.InsertData(
                table: "ServiceUsedByTicket",
                columns: new[] { "SUBTId", "CreateDate", "ServiceId", "Status", "TicketId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7697), "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 27, 0, 27, 42, 169, DateTimeKind.Local).AddTicks(7701), "2", 1, "2", null }
                });
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "BookingByRevenue",
                keyColumn: "BookingByRevenueId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "CustomerSupport",
                keyColumn: "CusSupportId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "DailyTicket",
                keyColumn: "DailyTicketId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "DailyTicket",
                keyColumn: "DailyTicketId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Feedback",
                keyColumn: "FeedbackId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Location",
                keyColumn: "LocationId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Notification",
                keyColumn: "NotifyId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PointOfInterest",
                keyColumn: "PointId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PointOfInterest",
                keyColumn: "PointId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Rate",
                keyColumn: "RateId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceByTourSegment",
                keyColumn: "SBTSId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceByTourSegment",
                keyColumn: "SBTSId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceType",
                keyColumn: "ServiceTypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceUsedByTicket",
                keyColumn: "SUBTId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceUsedByTicket",
                keyColumn: "SUBTId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TransactionsHistory",
                keyColumn: "TransactionId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Location",
                keyColumn: "LocationId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "POIType",
                keyColumn: "POITypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "POIType",
                keyColumn: "POITypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "RequestType",
                keyColumn: "RequestTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Revenue",
                keyColumn: "RevenueId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Service",
                keyColumn: "ServiceId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Service",
                keyColumn: "ServiceId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Ticket",
                keyColumn: "TicketId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Ticket",
                keyColumn: "TicketId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TicketType",
                keyColumn: "TicketTypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TourSegment",
                keyColumn: "TourSegmentId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "TourSegment",
                keyColumn: "TourSegmentId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Booking",
                keyColumn: "BookingId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Destination",
                keyColumn: "DestinationId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Destination",
                keyColumn: "DestinationId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "PackageTour",
                keyColumn: "PackageTourId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceType",
                keyColumn: "ServiceTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Supplier",
                keyColumn: "SupplierId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Supplier",
                keyColumn: "SupplierId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TicketType",
                keyColumn: "TicketTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Account",
                keyColumn: "Id",
                keyValue: "7e8f8c33-6506-4c2d-9258-c01f8c69413b");

            migrationBuilder.DeleteData(
                table: "City",
                keyColumn: "CityId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "DailyTour",
                keyColumn: "DailyTourId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PaymentMethod",
                keyColumn: "PaymentId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PackageTour",
                keyColumn: "PackageTourId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "City",
                keyColumn: "CityId",
                keyValue: "1");

            migrationBuilder.AddColumn<string>(
                name: "DailyTicketId",
                table: "DailyTour",
                type: "nvarchar(max)",
                nullable: true);
        }
    }
}
