using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

#pragma warning disable CA1814 // Prefer jagged arrays over multidimensional

namespace BusinessObjects.Migrations
{
    /// <inheritdoc />
    public partial class SeedData_01_09122024 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropColumn(
                name: "DailyTicketId",
                table: "DailyTour");

            migrationBuilder.InsertData(
                table: "Account",
                columns: new[] { "Id", "AccessFailedCount", "Address", "AvatarUrl", "ConcurrencyStamp", "CreateDate", "Dob", "Email", "EmailConfirmed", "FullName", "Gender", "LockoutEnabled", "LockoutEnd", "NormalizedEmail", "NormalizedUserName", "PasswordHash", "PhoneNumber", "PhoneNumberConfirmed", "RefreshToken", "RefreshTokenExpiryTime", "Roles", "SecurityStamp", "Status", "TwoFactorEnabled", "UpdateDate", "UserName", "ZaloUser" },
                values: new object[] { "a2fd6b0a-06e5-45a9-892c-700be351ac40", 0, "Quận 10, Hồ chí minh", "data:image/png;base64,iVBO", "9f981896-2de5-46f0-a540-5ad6747d2337", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(777), new DateTime(1990, 5, 15, 0, 0, 0, 0, DateTimeKind.Unspecified), "huahao04@gmail.com", false, "Hứa Thượng Hảo", true, false, null, null, null, null, "0395840777", false, null, null, null, "fbf0fc5a-1eaf-438a-a1bd-022ed98bf81f", 0, false, null, "admin", "" });

            migrationBuilder.InsertData(
                table: "City",
                columns: new[] { "CityId", "CityName", "CreateDate", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "Hồ Chí Minh", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(394), 0, null },
                    { "2", "Hà Nội", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(409), 1, null }
                });

            migrationBuilder.InsertData(
                table: "POIType",
                columns: new[] { "POITypeId", "CreateDate", "POITypeName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1153), "Chợ", 0, null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1158), "Bảo tàng", 1, null }
                });

            migrationBuilder.InsertData(
                table: "PaymentMethod",
                columns: new[] { "PaymentId", "CreateDate", "PaymentType", "Status", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2168), "Momo", 0, null });

            migrationBuilder.InsertData(
                table: "RequestType",
                columns: new[] { "RequestTypeId", "CreateDate", "Priority", "Status", "Type", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2373), 1, 0, "Support", null });

            migrationBuilder.InsertData(
                table: "Revenue",
                columns: new[] { "RevenueId", "CreateDate", "RevenueDate", "Status", "TotalRevenue", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2131), new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2129), 0, 900000f, null });

            migrationBuilder.InsertData(
                table: "ServiceType",
                columns: new[] { "ServiceTypeId", "CreateDate", "ServiceTypeName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(880), "Tour", 0, null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(885), "Hotel", 1, null }
                });

            migrationBuilder.InsertData(
                table: "Supplier",
                columns: new[] { "SupplierId", "CreateDate", "Status", "SupplierName", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(844), 0, "FPT", null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(847), 1, "VNPT", null }
                });

            migrationBuilder.InsertData(
                table: "CustomerSupport",
                columns: new[] { "CusSupportId", "CreateDate", "DateResolved", "Description", "RequestTypeId", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2345), new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2344), "Support", "1", 0, null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "Destination",
                columns: new[] { "DestinationId", "CityId", "CreateDate", "DestinationName", "PriceDestination", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1034), "Quận 1", 390000f, 0, null },
                    { "2", "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1039), "Quận 2", 350000f, 1, null }
                });

            migrationBuilder.InsertData(
                table: "Notification",
                columns: new[] { "NotifyId", "CreateDate", "Message", "SendDate", "Status", "Title", "Type", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2305), "Chúc mừng bạn đã đặt tour thành công", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2301), 0, "Thành công", "Success", null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "PackageTour",
                columns: new[] { "PackageTourId", "CityId", "CreateDate", "PackageTourName", "PackageTourPrice", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1753), "Tour Hồ Chí Minh", 500000f, 0, null },
                    { "2", "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1757), "Tour Hà Nội", 600000f, 1, null }
                });

            migrationBuilder.InsertData(
                table: "Service",
                columns: new[] { "ServiceId", "CreateDate", "LocationId", "ServiceName", "ServicePrice", "ServiceTypeId", "Status", "SupplierId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(919), null, "Tour Hồ Chí Minh", null, "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(922), null, "Tour Hà Nội", null, "1", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "DailyTour",
                columns: new[] { "DailyTourId", "CreateDate", "DailyTourName", "DailyTourPrice", "Description", "Discount", "EndDate", "ImgUrl", "PackageTourId", "StartDate", "Status", "UpdateDate" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1865), "Tour Hồ Chí Minh", 790000f, "Tour tham quan du lịch tp Hồ Chí Minh", 10, new DateTime(2024, 9, 12, 17, 30, 0, 0, DateTimeKind.Unspecified), "data:image/jpeg;base64,", "1", new DateTime(2024, 9, 12, 14, 30, 0, 0, DateTimeKind.Unspecified), 0, null });

            migrationBuilder.InsertData(
                table: "Location",
                columns: new[] { "LocationId", "CreateDate", "DestinationId", "LocationName", "LocationType", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1079), "1", "Nhà hàng", 1, 0, null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1082), "1", "Khách sạn", 1, 1, null }
                });

            migrationBuilder.InsertData(
                table: "TicketType",
                columns: new[] { "TicketTypeId", "CreateDate", "PackageTourId", "Status", "TicketTypeName", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1902), "1", 0, "Vé người lớn (>=16 tuổi)", null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1905), "1", 1, "Vé trẻ em", null }
                });

            migrationBuilder.InsertData(
                table: "TourSegment",
                columns: new[] { "TourSegmentId", "CreateDate", "DestinationId", "PackageTourId", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(994), "1", "1", 0, null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(996), "2", "2", 1, null }
                });

            migrationBuilder.InsertData(
                table: "Booking",
                columns: new[] { "BookingId", "BookingDate", "CreateDate", "DailyTourId", "PaymentId", "Status", "TotalPrice", "UpdateDate", "UserId" },
                values: new object[] { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1954), new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1959), "1", "1", 0, 500000f, null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "DailyTicket",
                columns: new[] { "DailyTicketId", "Capacity", "CreateDate", "DailyTourId", "Status", "TicketTypeId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", 10, new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1801), "1", 0, "1", null },
                    { "2", 10, new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1804), "1", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "PointOfInterest",
                columns: new[] { "PointId", "CreateDate", "LocationId", "POITypeId", "PointName", "Status", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1118), "1", "1", "Chợ Bến Thành", 0, null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(1121), "1", "2", "Bảo tàng Hồ Chí Minh", 1, null }
                });

            migrationBuilder.InsertData(
                table: "ServiceByTourSegment",
                columns: new[] { "SBTSId", "CreateDate", "ServiceId", "Status", "TourSegmentId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(956), "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(959), "2", 1, "2", null }
                });

            migrationBuilder.InsertData(
                table: "BookingByRevenue",
                columns: new[] { "BookingByRevenueId", "BookingId", "CreateDate", "RevenueId", "Status", "UpdateDate" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2086), "1", 0, null });

            migrationBuilder.InsertData(
                table: "Feedback",
                columns: new[] { "FeedbackId", "BookingId", "CreateDate", "FeedbackMsg", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2239), "Rất tuyệt vời", 0, null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "Rate",
                columns: new[] { "RateId", "BookingId", "CreateDate", "RateStar", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2270), 5, 0, null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "Ticket",
                columns: new[] { "TicketId", "BookingId", "CreateDate", "Price", "QR", "Quantity", "Status", "TicketName", "TicketTypeId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2004), 100000f, "", 10, 0, "Vé vào cổng", "1", null },
                    { "2", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2008), 50000f, "", 10, 1, "Vé ăn uống", "1", null }
                });

            migrationBuilder.InsertData(
                table: "TransactionsHistory",
                columns: new[] { "TransactionId", "BookingId", "CreateDate", "Status", "UpdateDate", "UserId" },
                values: new object[] { "1", "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2200), 0, null, "a2fd6b0a-06e5-45a9-892c-700be351ac40" });

            migrationBuilder.InsertData(
                table: "ServiceUsedByTicket",
                columns: new[] { "SUBTId", "CreateDate", "ServiceId", "Status", "TicketId", "UpdateDate" },
                values: new object[,]
                {
                    { "1", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2043), "1", 0, "1", null },
                    { "2", new DateTime(2024, 9, 12, 16, 54, 12, 24, DateTimeKind.Local).AddTicks(2047), "2", 1, "2", null }
                });
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "BookingByRevenue",
                keyColumn: "BookingByRevenueId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "CustomerSupport",
                keyColumn: "CusSupportId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "DailyTicket",
                keyColumn: "DailyTicketId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "DailyTicket",
                keyColumn: "DailyTicketId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Feedback",
                keyColumn: "FeedbackId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Location",
                keyColumn: "LocationId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Notification",
                keyColumn: "NotifyId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PointOfInterest",
                keyColumn: "PointId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PointOfInterest",
                keyColumn: "PointId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Rate",
                keyColumn: "RateId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceByTourSegment",
                keyColumn: "SBTSId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceByTourSegment",
                keyColumn: "SBTSId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceType",
                keyColumn: "ServiceTypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceUsedByTicket",
                keyColumn: "SUBTId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "ServiceUsedByTicket",
                keyColumn: "SUBTId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TransactionsHistory",
                keyColumn: "TransactionId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Location",
                keyColumn: "LocationId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "POIType",
                keyColumn: "POITypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "POIType",
                keyColumn: "POITypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "RequestType",
                keyColumn: "RequestTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Revenue",
                keyColumn: "RevenueId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Service",
                keyColumn: "ServiceId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Service",
                keyColumn: "ServiceId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Ticket",
                keyColumn: "TicketId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Ticket",
                keyColumn: "TicketId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TicketType",
                keyColumn: "TicketTypeId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TourSegment",
                keyColumn: "TourSegmentId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "TourSegment",
                keyColumn: "TourSegmentId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "Booking",
                keyColumn: "BookingId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Destination",
                keyColumn: "DestinationId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Destination",
                keyColumn: "DestinationId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "PackageTour",
                keyColumn: "PackageTourId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "ServiceType",
                keyColumn: "ServiceTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Supplier",
                keyColumn: "SupplierId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Supplier",
                keyColumn: "SupplierId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "TicketType",
                keyColumn: "TicketTypeId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "Account",
                keyColumn: "Id",
                keyValue: "a2fd6b0a-06e5-45a9-892c-700be351ac40");

            migrationBuilder.DeleteData(
                table: "City",
                keyColumn: "CityId",
                keyValue: "2");

            migrationBuilder.DeleteData(
                table: "DailyTour",
                keyColumn: "DailyTourId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PaymentMethod",
                keyColumn: "PaymentId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "PackageTour",
                keyColumn: "PackageTourId",
                keyValue: "1");

            migrationBuilder.DeleteData(
                table: "City",
                keyColumn: "CityId",
                keyValue: "1");

            migrationBuilder.AddColumn<string>(
                name: "DailyTicketId",
                table: "DailyTour",
                type: "nvarchar(max)",
                nullable: true);
        }
    }
}
